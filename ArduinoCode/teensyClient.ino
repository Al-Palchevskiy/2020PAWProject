/* Author(s):       Ahmad Naveed
 *                  Kevin Ta
 *                  Al Palchevskiy
 * Revision date:   2020 Jul 1st
 * Description:     Collects inertial measurement unit data, and hall data then transmits
 *                  through bluetooth to (Computer / Raspberry Pi).
 */
...
const int pushSensorPin = A1;       // Push input hall effect sensor
int pushSensorRead = 0;
float pushSensorVolt = 0;
int pushSensorInv = 0;
...
void setup() {
  ...
  analogReference(1);  //set adc ref voltage to 1.2v
  analogReadResolution(13); // 13 bit resolution
...
//    mpu.calibrateMPU6050v2(gyroBias, accelBias); // Calibrate gyro and accelerometers, load biases in bias registers  
    ...
    mpu.initMPU6050(); Serial.print("MPU6050 initialized for active data mode..."); // Initialize device for active mode read of acclerometer, gyroscope, and temperature
...
//  Serial.print(mpu.readByte(MPU6050_ADDRESS, ACCEL_CONFIG));
  bluetooth.begin(230400); // Serial1.begin - rx1 and tx1 = pins 0 and 1 on Teensy
...
//      temperature = ((float) tempCount) / 340. + 36.53; // Temperature in degrees Centigrade    
      ...
      pushSensorRead = analogRead(pushSensorPin);  //read push sensor input
      pushSensorVolt = map(pushSensorRead, 0, 8192, 0, 1200); // converting from 13 bit to millivolts 
      pushSensorInv = map(pushSensorRead, 0, 8192, -4096, 4096); // converting to inverting value.
    }  
//
//    count += 1;
//    
...
//    //*****Send message over BT*****//
    ...
    pbMsgGenerator.addHallData(pushSensorInv);

    // Get length of message
    int lengthf = pbMsgGenerator.generatePBMessage();

    //Serial.print("sending data: "); Serial.println(pushSensorVolt);
...
void rcvData(){
  // Receives data through Bluetooth
  String receivedText = "";
...
void sendATCmd(){ 
  // Send AT commands to program HC05?
